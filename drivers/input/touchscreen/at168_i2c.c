...
static void at168_work(struct work_struct *work)
{
	...
	if (event.data.fingers) {
  		input_report_abs(touch->input_dev, ABS_X,
			event.data.coord[0][0]);
		input_report_abs(touch->input_dev, ABS_Y,
			event.data.coord[0][1]);
  	}
	...
	const struct i2c_device_id *id)
{
	...
	clear_bit(EV_KEY, touch->input_dev->evbit); 
