...
{
        struct tegra_audio_data *audio_data = codec->control_data;

        pr_info("%s: new_con 0x%X #####################\n", __func__, new_con);

        /* Disconnect old codec routes and connect new routes*/
        if (new_con & TEGRA_HEADPHONE)
                snd_soc_dapm_enable_pin(&codec->card->dapm, "Headphone Jack");
        else
                snd_soc_dapm_disable_pin(&codec->card->dapm, "Headphone Jack");

        if (new_con & (TEGRA_SPK | TEGRA_EAR_SPK))
                snd_soc_dapm_enable_pin(&codec->card->dapm, "Internal Speaker");
        else
                snd_soc_dapm_disable_pin(&codec->card->dapm, "Internal Speaker");

        if (new_con & TEGRA_INT_MIC)
                snd_soc_dapm_enable_pin(&codec->card->dapm, "MIC1");
        else
                snd_soc_dapm_disable_pin(&codec->card->dapm, "MIC1");
...
static int tegra_codec_probe(struct snd_soc_codec *codec)
{
	struct tegra_audio_data* audio_data = codec->control_data;
	...
	if (!audio_data->init_done) {
		...
		snd_soc_dapm_new_controls(&codec->card->dapm, tegra_dapm_widgets,
					ARRAY_SIZE(tegra_dapm_widgets));

		/* Set up tegra specific audio path audio_map */
		snd_soc_dapm_add_routes(&codec->card->dapm, audio_map,
					ARRAY_SIZE(audio_map));

                /* Set endpoints to not connected */
                snd_soc_dapm_nc_pin(&codec->card->dapm, "LINEOUT");
                snd_soc_dapm_nc_pin(&codec->card->dapm, "LINEOUTN");
                snd_soc_dapm_nc_pin(&codec->card->dapm, "LINEINL");
                snd_soc_dapm_nc_pin(&codec->card->dapm, "LINEINR");
                snd_soc_dapm_nc_pin(&codec->card->dapm, "AUXINL");
                snd_soc_dapm_nc_pin(&codec->card->dapm, "AUXINR");
                snd_soc_dapm_nc_pin(&codec->card->dapm, "MIC2");

                /* Set endpoints to default off mode */
                snd_soc_dapm_enable_pin(&codec->card->dapm, "Internal Speaker");
                snd_soc_dapm_enable_pin(&codec->card->dapm, "Internal Mic");
                snd_soc_dapm_disable_pin(&codec->card->dapm, "Headphone Jack");

                ret = snd_soc_dapm_sync(&codec->card->dapm);
                if (ret) {
...
/*static struct snd_soc_dai_link tegra_soc_dai[] = {
	{
		...
		.cpu_dai_name = "tegra-alc5623",
		.codec_dai_name = "alc5623-hifi",
		.platform_name = "tegra-pcm-audio",
		.codec_name = "alc5623-codec",
		.init = &tegra_codec_init,
	},
	{
		.name = "Tegra-generic",
		.stream_name = "Tegra-generic AUX",
		.cpu_dai_name = "tegra-generic-aux",
		.codec_dai_name ="generic-aux",
		.platform_name = "tegra-pcm-audio",
	...
	},
	{
		...
		.codec_dai_name ="tegra-spdif",
		.platform_name = "tegra-pcm-audio",
		.codec_name = "spdif-codec",
	},
};*/
...
static struct snd_soc_card tegra_snd_soc = {
	.name = "tegra-alc5623",
	.dai_link 	= alc5623_dai,
	.num_links 	= ARRAY_SIZE(alc5623_dai),
...
static struct snd_soc_codec_driver soc_codec_dev_tegra = {
	.probe =	tegra_codec_probe,
	.remove =	tegra_codec_remove,
	.suspend =	tegra_suspend,
	.resume =	tegra_resume,
	.read = tegra_read,
	.write = tegra_write,
	.set_bias_level = tegra_set_bias_level,
};

static int __devinit tegra_probe(struct platform_device *pdev)
{
	return snd_soc_register_codec(&pdev->dev, &soc_codec_dev_tegra,
			&wm8400_dai, 1);
}

static int __devexit tegra_remove(struct platform_device *pdev)
{
	snd_soc_unregister_codec(&pdev->dev);
	return 0;
}

static struct platform_driver tegra_codec_driver = {
	.driver = {
		   .name = "acl5623-codec",
		   .owner = THIS_MODULE,
		   },
	.probe = tegra_probe,
	.remove = __devexit_p(tegra_remove),
};

static int __init tegra_init(void)
{
	return platform_driver_register(&tegra_codec_driver);
/*        int ret = 0;
